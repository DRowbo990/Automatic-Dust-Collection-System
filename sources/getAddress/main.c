...
*/
int main(void){							//begin
	unsigned char write0[4] = {'A', 'T', 13, '\n'};
	unsigned char write[10] = {'A', 'T', '+', 'A', 'D', 'D', 'R', '?', 13,'\n'};
	...
	USART_Write(USART3, write, 10);		//write AT+ADDR?
	USART_Read(USART3, read, 21);
	...
	while(1){ }/*
		USART_Write(USART2, read, 26);	//write original contents from UART
		USART_Write(USART3, write0, 4);		//write AT
		USART_Read(USART3, read, 2);
		USART_Write(USART2, read, 17);
		USART_Write(USART3, write0, 4);		//write AT
		USART_Read(USART3, read, 2);
		USART_Write(USART2, read, 17);
		USART_Write(USART3, write, 10);		//write("AT+ADDR?\r\n") to get address
		USART_Read(USART2, read, 14);
		USART_Write(USART2, read, 16);	//read from UART
	}*/
